...
class ObstacleManager {
  ...
  update(gameSpeed, score, delta) {
    this.spawnTimer += delta;

    // Time to spawn new obstacle
    if (this.spawnTimer >= this.nextSpawnTime) {
      this.spawnObstacle(score, gameSpeed);
      this.spawnTimer = 0;
  ...
  spawnObstacle(score, gameSpeed) {
    ...
    // Determine obstacle pool based on score
    ...
    this.obstacles.push(new Obstacle(randomType, gameSpeed));
  }
...
class CollectibleManager {
  ...
  update(gameSpeed, player, delta, pm) {
    this.spawnTimer += delta;
  ...
  spawnItem() {
    ...
    if (randomChance < POWERUP_SPAWN_CHANCE) {
      ...
      // Spawn a wave of coins (usually in rows/arcs of 3-5 coins)
      ...
      for (let i = 0; i < coinCount; i++) {
        ...
        if (isArc) {
          // Arc formula: mid point raises height
...
class Game {
  ...
  update(delta) {
    if (this.state === 'PLAYING') {
      ...
      this.om.update(this.gameSpeed, this.score, delta);
      this.cm.update(this.gameSpeed, this.player, delta, this.pm);
...
// Instantiate and kick off game loop once DOM is ready
function initGame() {
  const game = new Game();
